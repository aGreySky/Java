...
public class JumpJumpHelper {
     ...
     */
    private int[] getHalmaAndBoardXYValue(File currentImage)
            throws IOException {
        ...
        for (int y = gameScoreBottomY; y < height; y++) {
            for (int x = 0; x < width; x++) {
                ...
                //根据RGB的颜色来识别棋子的位置，  
                if (rValue > 50 && rValue < 60 && gValue > 53 && gValue < 63
                        && bValue > 95 && bValue < 110) {
                    halmaXSum += x;
                    halmaXCount++;
                    //棋子底行的Y坐标值  
        ...
        if (halmaXSum != 0 && halmaXCount != 0) {
            ...
            for (int y = gameScoreBottomY; y < height; y++) {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    if ((Math.abs(pixelR - lastPixelR)
                            + Math.abs(pixelG - lastPixelG)
            ...
            boardY = halmaY - Math.abs(boardX - halmaX)
    ...
    private void replayGame() {
        String command = String.format("adb shell input tap %s %s", swipeX,
                swipeY);
        executeCommand(command);
    }
     ...
     */
    private double computeJumpDistance(int halmaX, int halmaY, int boardX,
            int boardY) {
        return Math.sqrt(Math.pow(Math.abs(boardX - halmaX), 2)
                + Math.pow(Math.abs(boardY - halmaY), 2));
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            for (;;) {
                ...
                //获取跳棋和底板的中心坐标  
                int[] result = jumpjumpHelper
                        .getHalmaAndBoardXYValue(currentImage);
                if (result == null) {
                    System.out.println(
                            "The result of method getHalmaAndBoardXYValue is null!");
                    continue;
                }
                int halmaX = result[0];
                int halmaY = result[1];
                int boardX = result[2];
                int boardY = result[3];
                System.out.println("halmaX: " + halmaX + ", halmaY: " + halmaY
                        + ", boardX: " + boardX + ", boardY: " + boardY);
                //计算跳跃的距离  
                double jumpDistance = jumpjumpHelper.computeJumpDistance(halmaX,
                        halmaY, boardX, boardY);
    ...
    private void checkDoReplay() {
        if (imageLength[0] > 0 && imageLength[0] == imageLength[1]
                && imageLength[1] == imageLength[2]
                && imageLength[2] == imageLength[3]
