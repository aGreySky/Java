(function(){
    ...
    function regValue(id,i){
        ...
        switch (id){
            ...
            case "login_user_name":
                flag=/^[a-zA-Z0-9_]{4,16}$/.test(value.replace(/[\u0391-\uFFE5]/g,"nn")) || /^((\(\d{2,3}\))|(\d{3}\-))?1[3,8,5]{1}\d{9}$/.test(value) || /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}/.test(value);
                id="user_name";
                break; 
            case "password":
            case "login_password":
    ...
    regEvent(document.getElementById("button"),"click",function(){
        if(index1!=0){
            alert("您的输入有误，请检查并重新输入！");
        } else{
        	$.ajax({
	    			url:"login",
	    			data:{username: $("#login_user_name").val(),
	    			...
	    			success:function(data){
		    					if (data.flag) {
		    						alert("登录成功...")
		    						document.forms[1].action = 'paging?Page=1';
